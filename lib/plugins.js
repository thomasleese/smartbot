...
module.exports = {
  ...
  google: function(bot, config) {
    var googleRegex = /^(google|search) (.*)/i
  ...
  wolfram: function(bot, config) {
    var wolframString = function(s) {
      ...
      if (s.split("\n").length === 1) {
        ...
        var rows = s.split(/\r\n|\n|\r/);
        var maximumColumnWidth = 0;
        rows = rows.map(function(row) {
          return row.split(/\|/).map(function(col) {
            var c = col.trim();
            if (c.length > maximumColumnWidth) {
              maximumColumnWidth = c.length;
            }
            return c;
          });
        });
        ...
        rows.forEach(function(row) {
          ...
          row.forEach(function(col) {
            result += " " + col;
            var count = maximumColumnWidth + 2 - col.length;
      ...
      if (query && query.length > 2) {
        w.query(query[2], function(err, result) {
          if (result && result.length > 1) {
            var a = wolframString(result[0]["subpods"][0]["value"]);
            var b = wolframString(result[1]["subpods"][0]["value"]);
