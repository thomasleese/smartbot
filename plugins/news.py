...
class Plugin:
    ...
    def on_respond(self, bot, msg, reply):
        topic = msg["match"][0]
        if topic:
            url = "https://ajax.googleapis.com/ajax/services/search/news?v=1.0&rsz=5&q={0}".format(urllib.parse.quote(topic))
        else:
        ...
        for i, story in enumerate(res["responseData"]["results"][:3]):
